...
class Cifar100DataReader():
    ...
    def _decode(self,datum,onehot):
        ...
        if onehot:
            for d,l in datum:
                if self.resize:
                    image = np.reshape(np.reshape(d,[3,1024]).T,[32,32,3])
                    image = scipy.misc.imresize(image, size=(70, 70), interp='bicubic')
                    rdata.append(image)   # 转变形状为：32*32*3
                else:
    ...
    def next_test_data(self):
        ...
        if self.test_batch_index<len(self.data_label_test)/self.test_batch_size:
            print ("test_batch_index:",self.test_batch_index )
            datum=self.data_label_test[self.test_batch_index*self.test_batch_size:(self.test_batch_index+1)*self.test_batch_size]
