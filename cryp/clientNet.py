...
class Vgg19:
    ...
    def build(self, train_mode=None):
        ...
        self.convbin4_2,self.convbin4_2_mean, self.convbin4_2_var  = self.conv_layer(self.convbin4_1, 512, 512, "conv4_2", True)
        self.convbin4_3,self.convbin4_3_mean, self.convbin4_3_var = self.conv_layer(self.convbin4_2, 512, 512, "conv4_3", True)
        self.convbin4_4,self.convbin4_4_mean, self.convbin4_4_var = self.conv_layer(self.convbin4_3, 512, 512, "conv4_4", True)
        ...
        # with tf.variable_scope("bin"):
        self.convbin5_1,self.convbin5_1_mean, self.convbin5_1_var= self.conv_layer(self.poolbin4, 512, 512, "conv5_1", True)
        self.convbin5_2,self.convbin5_2_mean, self.convbin5_2_var = self.conv_layer(self.convbin5_1, 512, 512, "conv5_2", True)
        self.convbin5_3,_,_ = self.conv_layer(self.convbin5_2, 512, 512, "conv5_3", True)
        self.convbin5_4,_,_ = self.conv_layer(self.convbin5_3, 512, 512, "conv5_4", True)
        self.poolbin5 = self.max_pool(self.convbin5_4, "pool5")


        # with tf.variable_scope("bin"):
        self.fcbin6 = self.fc_layer(self.poolbin5, 512, 4096, "fc6",True)
    ...
    def conv_layer(self, bottom, in_channels, out_channels, name, binary=False):
        ...
        with tf.variable_scope(name):
            ...
            bn_layer, batch_mean, batch_var = self.batch_norm(conv, beta, gamma,name,out_channels, False)
            relu = tf.nn.relu(bn_layer)
            return relu, batch_mean, batch_var
    ...
    def get_constant_conv_var(self, filter_size, in_channels, out_channels, name):
        ...
        #filters =  tf.constant(np.loadtxt('../bin-weights-back/' + name + '_' +  name + '_filters.txt', dtype=np.float32).reshape(filterShape))
    ...
    def get_constant_fc_var(self, in_size, out_size, name):
        ...
        #weights =  tf.constant(np.loadtxt('../bin-weights-back/' + name + '_' + name + '_weights.txt', dtype=np.float32).reshape(weightShape))
